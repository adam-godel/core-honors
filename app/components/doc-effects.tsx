...
export function DocRewriteProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <DocRewriteContext.Provider value={{ state, triggerRewrite, allTyped, markAllTyped }}>
      {children}
...
function countChars(node: React.ReactNode): number {
  ...
  if (React.isValidElement(node))
    return countChars((node.props as Record<string, unknown>).children as React.ReactNode);
  return 0;
}
...
function renderPartial(
  ...
  if (node === null || node === undefined || node === false) return { content: null, consumed: 0 };
  ...
  if (Array.isArray(node)) {
    ...
    return { content: parts.length > 0 ? parts : null, consumed: count - remaining };
  }
  if (React.isValidElement(node)) {
    const nodeChildren = (node.props as Record<string, unknown>).children as React.ReactNode;
    ...
    return {
      content: React.cloneElement(node as React.ReactElement, {}, partial ?? ""),
      consumed,
    };
  }
...
function TypingBlock({
  ...
  const element = React.isValidElement(children)
    ...
    : null;
  const innerContent = element ? (element.props.children as React.ReactNode) : children;
  ...
  if (status === "done" || fullyTyped) return <div ref={ref} className="doc-tb">{children}</div>;

  if (status === "pending") {
    if (compact) return null;
    return <div ref={ref} className="doc-tb" style={{ visibility: "hidden" }}>{children}</div>;
  }

  // Typing: partial content + inline cursor
  const { content: partial } = renderPartial(innerContent, typedCount);
  const cursorEl = <span key="__cursor" className="doc-cursor" style={CURSOR_STYLE} />;

  if (element) {
    return <div ref={ref} className="doc-tb">{React.cloneElement(element, {}, partial ?? "", cursorEl)}</div>;
  }
  return <div ref={ref} className="doc-tb">{partial}{cursorEl}</div>;
}
...
export function DocRefreshButton() {
  ...
  return (
    ...
    >
      <span style={{ fontSize: "inherit", lineHeight: "inherit", color: "#80868b", fontStyle: "italic" }}>
        Clean this up?
      </span>
      <button
  ...
  return (
    <div>
      {childArray.map((child, i) => (
        <TypingBlock
          key={i}
          status={i < activeIndex ? "done" : i === activeIndex ? "typing" : "pending"}
          compact={true}
          startImmediately={true}
          onDone={() => setActiveIndex((prev) => Math.max(prev, i + 1))}
        >
